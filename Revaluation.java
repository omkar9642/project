...
public class Revaluation extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		while(rs.next()){
			...
			for(int x=0;x<6;x++){
				if(tcode.equals(acode+(x+1))||tcode.equals(sem+""+(x+1))||tcode.equals(acode1+(x+1))){
					String tex,tin;
					tex=rs.getString("External");
					if(!(tex.equals("-")||tex.equals(" ")||tex.equals(""))){
						exs[x]=tex;
						System.out.println(exs[x]+""+ins[x]);
					}
				}
			}
		}
		...
		while(rs.next()){
			for(int g=0;g<9;g++){
				credits[g]=Integer.parseInt(rs.getString(g+2));
			}		
		}
		rs.close();
			for(int h=0;h<9;h++){
			if(ex[h]>=24&&(ex[h]+in[h])>=90)
				gp[h]=10;
			else if(ex[h]>=24&&(ex[h]+in[h])>=80)
				gp[h]=9;
			else if(ex[h]>=24&&(ex[h]+in[h])>=70)
				gp[h]=8;
			else if(ex[h]>=24&&(ex[h]+in[h])>=60)
				gp[h]=7;
			else if(ex[h]>=24&&(ex[h]+in[h])>=50)
				gp[h]=6;
			else if(ex[h]>=24&&(ex[h]+in[h])>=40)
				gp[h]=5;
			else
				gp[h]=0;
		}
