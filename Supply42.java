...
public class Supply42 extends HttpServlet {
	...
	semtable+=sem;
		...
		rs.close();
			for(int w=0;w<6;w++){
			if(ex[w]>=24&&(ex[w]+in[w])>=90&&(ex[w]+in[w])<=100)
				gp[w]=10;
			else if(ex[w]>=24&&(ex[w]+in[w])>=80)
				gp[w]=9;
			else if(ex[w]>=24&&(ex[w]+in[w])>=70)
				gp[w]=8;
			else if(ex[w]>=24&&(ex[w]+in[w])>=60)
				gp[w]=7;
			else if(ex[w]>=24&&(ex[w]+in[w])>=50)
				gp[w]=6;
			else if(ex[w]>=24&&(ex[w]+in[w])>=40)
				gp[w]=5;
			else 
				gp[w]=0;
		}	
