...
public class Y14Regular extends HttpServlet {
static int[] credits={0,0,0,0,0,0,0,0,0};
	...
	public static void reval(String b,int sem)throws Exception{
		rs=stmt.executeQuery("select * from [rvreg$] where REGDNO='"+regno+"'");
		while(rs.next()){
			String tcode="",acode=b+" "+sem,acode1=b+""+sem;
			tcode=rs.getString("CODE");
			for(int i=0;i<6;i++){
				if(tcode.equals(acode+(i+1))||tcode.equals(sem+""+(i+1))||tcode.equals(acode1+(i+1))){
					String tex,tin;
					...
					if(!(tex.equals("-")||tex.equals(" ")||tex.equals(""))){
						exs[i]=tex;
						//ins[i]=tin;
						System.out.println(exs[i]+""+ins[i]);
						ex[i]=Integer.parseInt(exs[i]);
